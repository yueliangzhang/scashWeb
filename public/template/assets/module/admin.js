layui.define(["layer"], function(f) {
    ...
    n.reloadLayer = function(p, i, r) {
        ...
        n.ajax({
            ...
            success: function(v) {
                ...
                n.renderTpl("#layui-layer" + p + " [mx-tpl]");
                r && r(q[0], p)
            }
        })
    };
    ...
    n.btnLoading = function(p, q, r) {
        ...
        var i = h(p);
        if (r) {
            i.addClass("mx-btn-loading");
            i.prepend('<span class="mx-btn-loading-text"><i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i>' + q + "</span>");
            i.attr("disabled", "disabled").prop("disabled", true)
        } else {
            i.removeClass("mx-btn-loading");
            i.children(".mx-btn-loading-text").remove();
    ...
    n.chooseLocation = function(t) {
                    ...
                    if (I === "complete") {
                        ...
                        for (var K = 0; K < L.length; K++) {
                            ...
                            if (J.location !== undefined) {
                                M += '<div data-lng="' + J.location.lng + '" data-lat="' + J.location.lat + '" class="mx-map-select-search-list-item">';
                                M += '     <div class="mx-map-select-search-list-item-title">' + J.name + "</div>";
                                M += '     <div class="mx-map-select-search-list-item-address">' + J.address + "</div>";
                                M += '     <div class="mx-map-select-search-list-item-icon-ok layui-hide"><i class="layui-icon layui-icon-ok-circle"></i></div>';
                                M += "</div>"
                            }
                        }
                        h("#mx-map-select-pois").html(M)
                    }
                })
            })
        };
        var i = function() {
            ...
            y && (D.center = y);
            var F = new AMap.Map("mx-map-select-map", D);
            ...
            F.on("moveend", function() {
                if (C) {
                    ...
                    h("#mx-map-select-tips").addClass("layui-hide");
                    h("#mx-map-select-center-img").removeClass("bounceInDown");
                    setTimeout(function() {
                        h("#mx-map-select-center-img").addClass("bounceInDown")
                    });
                    var G = F.getCenter();
                    v(G.lat, G.lng)
                }
            });
            h("#mx-map-select-pois").off("click").on("click", ".mx-map-select-search-list-item", function() {
                h("#mx-map-select-tips").addClass("layui-hide");
                h("#mx-map-select-pois .mx-map-select-search-list-item-icon-ok").addClass("layui-hide");
                h(this).find(".mx-map-select-search-list-item-icon-ok").removeClass("layui-hide");
                h("#mx-map-select-center-img").removeClass("bounceInDown");
                setTimeout(function() {
                    h("#mx-map-select-center-img").addClass("bounceInDown")
                });
                ...
                var H = h(this).find(".mx-map-select-search-list-item-title").text();
                var G = h(this).find(".mx-map-select-search-list-item-address").text();
                z = {
                    name: H,
                    address: G,
                    lat: J,
            ...
            h("#mx-map-select-btn-ok").click(function() {
                if (z === undefined) {
                    k.msg("请点击位置列表选择", {
                        icon: 2,
                        anim: 6
                    ...
                    if (x) {
                        if (r) {
                            ...
                            F.getCity(function(H) {
                                k.close(G);
                                z.city = H;
                                n.closeDialog("#mx-map-select-btn-ok");
                                x(z)
                            })
                        } else {
                            n.closeDialog("#mx-map-select-btn-ok");
            ...
            var E = h("#mx-map-select-input-search");
            E.off("input").on("input", function() {
                var G = h(this).val();
                var H = h("#mx-map-select-tips");
                if (!G) {
                    H.html("");
                        ...
                        if (K.tips) {
                            ...
                            for (var M = 0; M < J.length; M++) {
                                ...
                                if (O.location !== undefined) {
                                    N += '<div data-lng="' + O.location.lng + '" data-lat="' + O.location.lat + '" class="mx-map-select-search-list-item">';
                                    N += '     <div class="mx-map-select-search-list-item-icon-search"><i class="layui-icon layui-icon-search"></i></div>';
                                    N += '     <div class="mx-map-select-search-list-item-title">' + O.name + "</div>";
                                    N += '     <div class="mx-map-select-search-list-item-address">' + O.address + "</div>";
                                    N += "</div>"
                                }
                            }
                            H.html(N);
                            if (J.length === 0) {
                                ...
                                h("#mx-map-select-tips").removeClass("layui-hide")
                            }
                        } else {
                            H.html("");
                            H.addClass("layui-hide")
            ...
            h("#mx-map-select-tips").off("click").on("click", ".mx-map-select-search-list-item", function() {
                h("#mx-map-select-tips").addClass("layui-hide");
        ...
        var q = ['<div class="mx-map-select-tool" style="position: relative;">', '     搜索：<input id="mx-map-select-input-search" class="layui-input icon-search inline-block" style="width: 190px;" placeholder="输入关键字搜索" autocomplete="off" />', '     <button id="mx-map-select-btn-ok" class="layui-btn icon-btn pull-right" type="button"><i class="layui-icon">&#xe605;</i>确定</button>', '     <div id="mx-map-select-tips" class="mx-map-select-search-list layui-hide">', "     </div>", "</div>", '<div class="layui-row mx-map-select">', '     <div class="layui-col-sm7 mx-map-select-map-group" style="position: relative;">', '          <div id="mx-map-select-map"></div>', '          <i id="mx-map-select-center-img2" class="layui-icon layui-icon-add-1"></i>', '          <img id="mx-map-select-center-img" src="https://3gimg.qq.com/lightmap/components/locationPicker2/image/marker.png" alt=""/>', "     </div>", '     <div id="mx-map-select-pois" class="layui-col-sm5 mx-map-select-search-list">', "     </div>", "</div>"].join("");
        n.open({
            id: "mx-map-select",
            type: 1,
            title: p,
    ...
    n.cropImg = function(r) {
        ...
        layui.use(["Cropper", "upload"], function() {
            ...
            function A() {
                var D, E = h("#mx-crop-img");
                var F = {
                    elem: "#mx-crop-img-upload",
                    auto: false,
                    drag: false,
                    choose: function(G) {
                ...
                y.render(F);
                if (!x) {
                    return h("#mx-crop-img-upload").trigger("click")
                }
                var C = {
                    aspectRatio: w,
                    preview: "#mx-crop-img-preview"
                };
                D = new z(E[0], C);
                h(".mx-crop-tool").on("click", "[data-method]", function() {
                    var H = h(this).data(),
                        I, G;
                    if (!D || !H.method) {
                        return
    ...
    n.util = {
        ...
        render: function(p) {
            ...
            h(p.elem).next("[mx-tpl-rs]").remove();
            h(p.elem).after(i);
            h(p.elem).next().attr("mx-tpl-rs", "");
            p.done && p.done(p.url)
        }
    };
        ...
        if (i.length > 0) {
            ...
            n.ajax({
                ...
                success: function(r) {
                    k.close(p);
                    if (typeof r === "string") {
                        h("body").append('<div id="mx-lock-screen-group">' + r + "</div>");
                        n.isLockScreen = true;
                        n.putTempData("isLockScreen", n.isLockScreen, true);
    ...
    n.renderTpl = function(p) {
        ...
        h(p || "[mx-tpl]").each(function() {
            var s = h(this);
            var q = n.util.deepClone(h(this).data());
            q.elem = s;
            q.tpl = s.html();
            q.url = i(s.attr("mx-tpl"));
    ...
    h(document).on("mouseenter", m + "," + m + " .layui-nav-child>dd", function() {
        ...
        i.removeClass("show-top mx-anim-drop-in");
        i.css({
            "left": "auto",
            "top": "auto"
        })
    });
    h(document).on("click", "*[mx-event]", function() {
        var i = n.events[h(this).attr("mx-event")];
    ...
    h(document).on("click.mx-sel-fixed", ".mx-select-fixed .layui-form-select .layui-select-title", function() {
        var v = h(this),
    ...
    n.hideFixedEl = function() {
        h(".mx-select-fixed .layui-form-select").removeClass("layui-form-selected layui-form-selectup");
    ...
    h(document).on("click", "*[mx-href]", function() {
        var s = h(this);
        var p = s.attr("mx-href");
        if (!p || p === "#") {
            return
        }
